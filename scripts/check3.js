        ...
        async function submitAuthPw() {
            ...
            if(authFlow === 'login') {
                ...
                if(!error && data.session) {
                    ...
                    // Fallback for legacy users without a password
                    const { data: legacy } = await window.supabaseClient.from('master_artists').select('id, name').eq('email', artistData.email).single();
                    if(legacy) {
                        const { data: suData, error: suError } = await window.supabaseClient.auth.signUp({ email: artistData.email, password: artistData.password });
                        if(!suError) {
                            addFloweeMessage(`I found your soul imprint, ${legacy.name}, but you didn't have a password yet! I have now secured your Sanctuary with the frequency you just entered. Redirecting...`);
                            setTimeout(() => { window.location.href = `/pages/artist_sanctuary.html?eventId=${eventId || ''}&artistId=${legacy.id}`; }, 4000);
                            return;
                        } else if(suError && !suError.message.includes("already registered")) {
                            addFloweeMessage("Error: " + suError.message);
                            setInputOptions(`
                ...
                if(error) {
                    if(error.message.includes("already registered")) {
                        // Switch to login flow for them implicitly
                        addFloweeMessage("Your soul is already registered! Please log in instead.");
                        authFlow = 'login';
                        setInputOptions(`
                            <div class="flex gap-2">
                                <input type="password" id="chat-input-pw" placeholder="${getT('pwPlaceholder')}" class="flex-1 bg-black/50 border border-white/20 rounded-lg p-3 text-white text-sm outline-none focus:border-[#00ffcc]">
