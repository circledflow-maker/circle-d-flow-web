...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!userId) {
      // Guest path: Bronze preview — coupons unlock at Silver (€5)
      let coupons = [];
      try {
        coupons = await listCouponsForTier(db, 'registered');
        // Partner coupons are Silver+ — hide from free Bronze claim UX
        coupons = (coupons || []).filter((c) => {
          const min = String(c.minTier || c.min_tier || 'registered');
    ...
    const patch = {
      ...
      member_card_source: source,
    };
    if (instagram) patch.instagram_handle = instagram;

    const { error: upErr } = await db.from('profiles').update(patch).eq('id', userId);
    if (upErr) {
      if (/member_card_source|column/i.test(upErr.message || '')) {
        delete patch.member_card_source;
        const { error: retrySrc } = await db.from('profiles').update(patch).eq('id', userId);
        if (retrySrc) {
          if (/instagram|column/i.test(retrySrc.message || '')) {
            delete patch.instagram_handle;
      ...
      } else if (/instagram|column/i.test(upErr.message || '')) {
        delete patch.instagram_handle;
        const { error: retryErr } = await db.from('profiles').update(patch).eq('id', userId);
        if (retryErr) throw retryErr;
      } else {
