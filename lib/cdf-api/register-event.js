 ...
 * Wako Kungo · Botanical Groove Session (Botânica Lisboa · 24 Sep) — Member & Jam.
...
async function softNotifyResend({ email, fullName, stageName, registrationId }) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      ...
      to: [email],
      subject: 'Welcome — Botanical Groove Session · Wako Kungo',
      text:
        `Hi ${name},\n\n` +
        `You are registered for ${EVENT_LABEL} (Member & Jam).\n` +
        `When: ${EVENT_WHEN}\n` +
        `Venue: ${EVENT_VENUE}\n` +
        `Entry: Free · Responsible donations\n` +
        `Registration id: ${registrationId}\n\n` +
        `Next: claim your Wako Kungo Membership Card, then enter the 3D Artist Sanctuary.\n` +
        `https://circle-d-flow-web.vercel.app/member-card\n\n` +
        `See you in the Flow,\nWako Kungo · Circle D Flow C4C`,
    });
    if (result.error) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const attendingAug29 = asBool(
      body.attendingAug29 ?? body.attending_aug29 ?? body.attendingEvent ?? body.attending
    );
    ...
    if (attendingAug29 === null) {
      return res.status(400).json({ error: 'Please indicate if you attend Botânica on 24/09' });
    }
