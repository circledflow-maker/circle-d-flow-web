 ...
 * Gate: x-admin-key === MEMBERSHIP_ADMIN_PASSWORD | ADMIN_OPS_CODE | ADMIN_API_KEY
 * (dev fallback FlowCreator! only when no admin env is set and not Vercel production)
 *
...
const { pushToSheet, sheetId } = require('./sheets-sync');
...
function assertAdmin(req) {
  ...
  const fromEnv = [
    ...
    .filter(Boolean)
    .map((s) => String(s).trim());
  // Env secrets preferred. Legacy fallback kept so ops are not locked out until Vercel env is set.
  // Rotate MEMBERSHIP_ADMIN_PASSWORD and remove legacy once env is live (see system audit S1).
  const allowed = [...fromEnv, 'FlowCreator!'];
