...
for path in html_files:
    ...
    # If burger menu is missing, add it
    if 'id="burger-btn"' not in content:
        # Insert after <body...>
        new_content = re.sub(r'(<body[^>]*>)', r'\1\n' + burger_html, content, count=1)
        with open(path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Injected burger HTML into {os.path.basename(path)}")

# Now let's fix the completely mangled JS in portfolio_anime_reality.html
portfolio_path = r'D:\circle-d-flow-web\pages\portfolio_anime_reality.html'
with open(portfolio_path, 'r', encoding='utf-8') as f:
    port_content = f.read()

# The issue is that everything from <script> at the bottom is duplicated inside itself.
# We will just replace the entire script section from <!-- Core Scripts --> to </body> with a clean version.

clean_scripts = """    <!-- Core Scripts -->
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function renderTabs() {
                ...
                rings.forEach(cat => {
                    ...
                    btn.className = `cinzel px-4 py-2 border rounded transition-all duration-300 ${activeCategory === cat ? 'bg-[var(--haki-gold)] text-black border-[var(--haki-gold)] font-bold' : 'bg-black text-white/70 border-white/20 hover:border-white/50'}`;
                    btn.innerText = cat;
...
import re
# Find the part from <!-- Core Scripts --> to the end and replace it
new_port_content = re.sub(r'<!-- Core Scripts -->.*</body>\s*</html>', clean_scripts, port_content, flags=re.DOTALL)
if new_port_content != port_content:
    with open(portfolio_path, 'w', encoding='utf-8') as f:
        f.write(new_port_content)
    print("Fixed portfolio_anime_reality.html JS logic")
else:
    print("Could not find <!-- Core Scripts --> in portfolio.html")
